...
//    the key to preexist.
//
// Mounted by ./contentEditor's shell provider (above <Routes>, so an open
// draft survives client-side navigation) — but ONLY while edit mode is
// active, as a lazy chunk: readers never download the editor. The context +
// useContentEditor hook live in the shell; this file publishes its api up
// through `onApi`.
import {
  useEffect,
  useRef,
...
import type { ContentEditorApi } from './contentEditor'
...
export default function ContentEditorImpl({
  onApi,
}: {
  onApi: (api: ContentEditorApi | null) => void
}) {
  ...
  // Publish ONE stable api object to the shell (its methods delegate to the
  // latest render's closures via apiRef); republishing a fresh object every
  // render would loop the shell's setState.
  const apiRef = useRef<ContentEditorApi>({ openFile, openDraft, setPostState, deleteFile })
  apiRef.current = { openFile, openDraft, setPostState, deleteFile }
  useEffect(() => {
    onApi({
      openFile: (opts) => apiRef.current.openFile(opts),
      openDraft: (opts) => apiRef.current.openDraft(opts),
      setPostState: (ref, next) => apiRef.current.setPostState(ref, next),
      deleteFile: (path) => apiRef.current.deleteFile(path),
    })
    return () => onApi(null)
  }, [onApi])
