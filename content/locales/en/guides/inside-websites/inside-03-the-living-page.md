...
> **Chapter 3 of 5 · The Path** — [← Chapter 2: From Markdown to Bundle](/guides/inside-02-from-markdown-to-bundle) · [Chapter 4: Shipping the Box →](/guides/inside-04-shipping-the-box)
...
`main.tsx` → `App` → `Layout` → `GuideDetail` → `Markdown` — and you know
...
| `/guides` | `Guides` | epics from `site.yaml`, guides by first tag |
| `/guides/:slug` | `GuideDetail` | one guide file — this page |
...
`GuideDetail` asks the content layer for that slug, and the tree repaints.
...
page's course tabs. `Guides.tsx` holds
...
`/guides/inside-03-the-living-page` *assumed the JavaScript was already
...
**[Chapter 4: Shipping the Box →](/guides/inside-04-shipping-the-box)**
