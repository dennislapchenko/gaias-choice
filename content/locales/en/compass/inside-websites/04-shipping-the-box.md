...
> **Chapter 4 of 5 · The Path** — [← Chapter 3: The Living Page](/compass/inside-websites/03-the-living-page) · [Chapter 5: Owning the Stack →](/compass/inside-websites/05-owning-the-stack)
...
Chapter 3 left a wire exposed — worth understanding even though this site now
covers it twice. Paste `/compass/inside-websites/03-the-living-page` into a
fresh tab. The browser sends that path to the server, which has **the
bundle**. Ask what file answers it. On a *naive* SPA build the answer is
"none" — there'd be only `index.html`, `assets/`, images, and every static
host in the world would 404 that deep link. That was this site's original
shape, and the classic SPA trap: it works perfectly until the first person
shares a link.

This site answers the same request **twice over**, belt and braces:

- **Prerendering** (chapter 5's payoff): the build writes a real
  `compass/inside-websites/03-the-living-page.html` for *every* route, so the
  file the naive build lacked genuinely exists. That deep link hits static
  HTML — good for the reader's first paint and good for crawlers.
- **The SPA fallback**, still in `nginx/default.conf.template` as the net
  underneath, for any path prerendering didn't emit:

  ```nginx
  try_files $uri $uri.html $uri/ /index.html;
  ```

  Try the path as a file, then as `<path>.html` (the prerendered page), then
  serve **the one HTML page** anyway. Whatever answers, the JavaScript boots,
  the router (chapter 3) reads the URL it loaded on, and takes over. Server
  and client split one job: the server hands back a page for *every* path, the
  client makes the path mean something.
...
3. Test the seed from chapter 3:
   `curl -i http://localhost:8080/compass/inside-websites/01-the-whole-machine` —
...
*chosen*, and for a long time it quietly owed a real debt: an SPA's shell is
nearly empty, so everything that reads pages without running JavaScript —
search crawlers, link previews, Pinterest — once saw almost nothing. Chapter
5 is where that debt gets named in full and then *paid*, with the
prerendering you already met in this chapter's fallback: how it works, why it
cost no architecture change, and where the walls of this whole design still
are.

**[Chapter 5: Owning the Stack →](/compass/inside-websites/05-owning-the-stack)**
