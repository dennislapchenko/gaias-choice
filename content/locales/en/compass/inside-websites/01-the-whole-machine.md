...
> **Chapter 1 of 5 · The Gate** — [Chapter 2: From Markdown to Bundle →](/compass/inside-websites/02-from-markdown-to-bundle)
...
Now the strange part — and one honest complication. Open the repo's
`frontend/index.html`, or view-source at the very first paint: underneath
everything is a **nearly empty** HTML document — a `<div id="root"></div>`
and a script tag. None of the text you are reading is authored *into* that
file.

That is because this is a **single-page application (SPA)**: at browse time
there is only that one shell — **the one HTML page** — and the JavaScript
builds everything you see, directly in your browser. When you click from
Reviews to Compass, no new page is fetched; the JavaScript swaps what's
painted inside that one page and rewrites the URL bar so it *looks* like you
navigated. The tell is the reload: real navigations flash and refetch; these
don't.

The complication: if you view-source *this* page you'll see a full HTML
document, not the empty shell. That's because the build **prerenders** every
route — it runs the same program once per URL and writes the painted HTML
into the bundle, so crawlers and first paint get a real page while the SPA
still takes over for humans. So both facts are true at once: the app *is* one
shell plus a program, and the shipped pages are full HTML. Chapter 3 opens
the browse-time machinery; chapter 4 ships the box; chapter 5 is where
prerendering — the payoff that reconciles these two facts — gets its full
accounting.

If you remember nothing else from this chapter: **the site you're browsing
is one HTML page plus a program**, compiled from the repository at build
time — and that same build renders the program to static HTML ahead of you.
...
- Describe what an SPA is — the empty shell underneath, the reload-free
  navigation on top — and explain why this site's page source is nevertheless
  full HTML (prerendering).
...
3. The `index.html` shell is nearly empty, yet this page's source is full
   HTML *and* it behaves like an SPA — how do both hold at once?
...
**[Chapter 2: From Markdown to Bundle →](/compass/inside-websites/02-from-markdown-to-bundle)**
