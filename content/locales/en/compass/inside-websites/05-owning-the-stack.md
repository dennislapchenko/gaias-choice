...
> **Chapter 5 of 5 · The Summit** — [← Chapter 4: Shipping the Box](/compass/inside-websites/04-shipping-the-box) · after this: [where to go next](#beyond-this-course)
...
| 6 | `src/components/Layout.tsx` → one page (`pages/Compass.tsx`) | tree spine + the one `useState` you already met (ch. 3) |
...
| | This site (SPA + prerender) | Static-site generator (Astro, Hugo…) | Server-rendered (Next.js…) | CMS (WordPress…) |
| --- | --- | --- | --- | --- |
| First HTML the reader gets | full page (prerendered)† | full page | full page | full page |
...
| What crawlers/previews see | full page (prerendered)† | everything | everything | everything |

† A *naive* SPA scores "nearly empty / almost nothing" on those two rows —
that was this site's original shape and its one real debt. Prerendering (the
row below) paid it: the top two cells now match the static-generator column,
...
everything, once prerendering closes the two rows that used to lose. But hold
the comparison honestly: for a *content site*, the static-site generator
column is the industry-standard answer, and it is a strong one — the honest
read is that prerendering makes this SPA *converge toward* what an Astro site
gives you out of the box. If you rebuild for yourself, decide per that table,
not per fashion — and know that this repo would have been almost as happy as
an Astro site.
...
## Where it broke — and how it was paid

Chapter 4's seed, now paid in full. Rewind to this site's original shape and
read a deep link as a crawler would: title, empty div, script tag. A search
engine that doesn't execute JavaScript, a social-preview fetcher, a Pinterest
bot — each saw a blank site. For an affiliate content business whose
chapter-3-of-the-*founder*-course traffic plan leans on search and Pinterest,
that was not a footnote; it was the architecture's one real debt.

The repayment is **build-time prerendering**, and it has shipped. Every route
is knowable at build time (the slugs are just filenames — chapter 2), so the
build visits each one, captures the painted HTML, and writes it into the
bundle: `dist/<path>.html` for every page of both locale trees, plus a
sitemap and per-page `<title>`/canonical/Open Graph heads
(`src/entry-server.tsx` + `scripts/prerender.mjs`, run inside `npm run
build`). Crawlers now get full pages; humans get the same instant SPA once
the JavaScript wakes up and hydrates over the prerendered HTML. It cost no
architecture change — one more build step, and the try_files line from
chapter 4 gained a `$uri.html` so the nginx box serves those files too. The
two-phase machine absorbed it without moving the wall, which is the whole
reason it was affordable.
...
- Name the stack's one real debt, how prerendering repaid it, and the limits
  that would still end this architecture.
...
1. What did a non-JavaScript crawler see on a *naive* build of this site, why
   does it matter *for this specific business*, and how does prerendering fix
...
[«An honest site from zero»](/compass/founder-guide/kickstart-playbook), five chapters,
same format. And the [roadmap](/roadmap) is where you watch this stack's next
chapters land in public — the way prerendering, taught above, already has.
