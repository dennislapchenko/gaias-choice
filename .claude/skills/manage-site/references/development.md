...
| **Hand-authored inline SVG icons (page chrome)** | `Campfire()`/`DragonRing()` in `src/pages/Account.tsx` + `.campfire*`/`.campfire-flames`/`.dragon-ring`/`.dragon-shimmer` in `styles.css` | Distinct from the Mandala generator above (that's for *content* images): this is the recipe for a one-off decorative icon baked directly into a component — a fixed `viewBox="0 0 120 120"`, shapes layered straight in JSX (paths/circles/polygons, colored via a `<linearGradient>` or a CSS var), optionally wrapped in a `<g className="…">` that a CSS `@keyframes` animates (Campfire's flame flicker/glow; DragonRing's `.dragon-shimmer` on the diamond/admin tier) — deliberately **not** gated behind `prefers-reduced-motion` for these small flourishes (owner wants them alive). A repeated motif (DragonRing's back spikes) reuses the Mandala generator's own trick instead of hand-plotting each copy: draw the shape once in `<defs>`, stamp it around a circle via `<use href="#id" transform="rotate(angle cx cy) translate(0 -r)">`. Reach for this whenever chrome needs its own small bespoke icon that isn't a raster asset and doesn't fit the mandala script — e.g. the per-role rank ring on `/account`'s campfire (`DragonRing`, one gradient per tier in `DRAGON_STOPS`, keyed by `Role`: silver/gold/diamond). |
...
- **Make one scalar flip instantly (a toggle, no dialog):** (1) add a
  ...
  control **only when** `useEditMode().active`, calling
  `useContentEditor().setScalar(ref, value)` and updating local state
  in `.then()` so the change is visible immediately (see `StateToggle.tsx`);
  (3) nothing else — `setScalar` handles fetch, YAML surgery (inserting the
  key if it's entirely absent), sha conflicts, and the commit.
