...
| **Login, accounts & the campfire** | Session seam: `src/lib/session.tsx` (`SessionProvider`/`useSession`, outermost in `App.tsx`; owns `localStorage['gc-session']` and the dialog state). UI: `src/components/UserButton.tsx` (header, between the language and palette switchers), `src/components/LoginDialog.tsx` (modal), `src/pages/Account.tsx` (`/account`), `src/components/AccountFields.tsx` (toggled panel). Types: `LoginResponse`/`MeResponse`/`Member`/`UpdateMePayload` in `lib/api.ts`. Styles: `.user-toggle`/`.login-*`/`.campfire*`/`.camper*`/`.account-header`/`.cube-toggle`/`.account-fields`/`.field*`/`.avatar-img`. Strings: `login.*`/`account.*` (en+ru). | All of it renders **only when the API answers** (`backendUp`: `/healthz` probe signed out, `/auth/me` signed in) — the pure static site shows none of it. Signed out the button opens the dialog (sign-in ↔ create-account toggle; register creates a **viewer** and signs straight in); signed in it shows the user's initial (or avatar image) and links to `/account`: the campfire, with every registered user placed around the fire by trig (`Account.tsx` computes seat angles; avatar = the user's `avatarUrl` image if set — cropped/zoomed via `.avatar-img` (`object-fit: cover; transform: scale(2)`), same crop approach as `.epic-row-img` — else the display-name initial cycling the pastel slots; `you` highlighted with the accent). The page title sits in a header row (`.account-header`) beside a square `.cube-toggle` button that shows/hides `AccountFields` — a self-service form (name, avatar URL, email, password) that flows inline above the campfire scene when open, rather than living in a persistent rail; this also sidesteps the double-sidebar mobile reflow (`.reviews-layout`'s `order:-1` rail-above-content rule), which used to shove the whole form above the fire on narrow screens. Toggling calls no API — it's local `useState` in `Account.tsx`. The form itself calls `PUT /api/users/me` (session-authed, any role) and syncs the result back into `session.me` via `useSession().updateMe()`; Save appears only once a field differs from the last-saved values, role is shown but disabled (admin-only to change), and email conflicts surface a 409 inline. Sign-out button sits below the scene. Login/save errors localize by HTTP status (401/409/400/429). Passwords are the stopgap — magic-link (SMTP) and WebAuthn are future work. |
