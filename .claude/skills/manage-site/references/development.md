...
| **Upcoming ("in the works" rail — Reviews + Journal)** | `src/components/Upcoming.tsx` + `.upcoming*`/`.reviews-layout`/`.reviews-main` in `styles.css`; data = posts with `state: upcoming` frontmatter via `getUpcomingProducts`/`getUpcomingJournal` (`lib/content.ts`) | Flat rectangle list in the **right-hand rail** of `/reviews` AND `/journal` — the public queue of unfinished posts. A WIP post is a **real content file** (`products/` or `journal/`) with `state: upcoming`; the rail shows its frontmatter `title` and nothing else (no rating, no date, no link for readers). Flip the frontmatter to `active` (or delete the line) and the post moves into the main listing — the file never moves. Both pages wrap their main column (`.reviews-main`) and the `<Upcoming>` aside in the same `.reviews-layout` grid (`1fr` + `clamp(240px,24vw,300px)`); the page header stays full-width above. On desktop the rail is `position: sticky` (top 84px); **≤900px** the layout becomes a flex column and the rail rides above the content (`order:-1`). Entries are faintly greyed (`rgba(47,44,39,.05)`) so they don't read as live cards. Title/note are **props** — each page passes its own strings (`reviews.upcomingTitle`/`upcomingNote`, `journal.upcomingTitle`/`upcomingNote`). The rail also hosts the **"Contribute!" copy-template button** at its bottom via the optional `contribute: { value, label }` prop (see the "Contribute!" row); with `contribute` set the rail renders even when `items` is empty. The getters merge en + locale **by slug** (locale wins): review stubs live in `en/products/` only (brand names are do-not-translate) and show on every locale's rail; journal idea titles ARE localized — give the ru stub the **same slug** as the en one. In **edit mode** rows link to the post's real URL (WIP preview — detail pages show an "In the works" tag) and a ＋ button under the list queues a new upcoming post (see the Live-edit row). See the **Double-sidebar pages** row for the wide-screen widen + dimming this rail shares with the entry-detail TOC. |
