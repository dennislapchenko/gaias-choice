...
| **Double-sidebar pages** (wide-screen widen + rail dimming) | `styles.css`: `.layout-grid:has(.toc, .upcoming)` / `.detail-layout:has(.toc)` | Any page where the site-wide left `.sidebar` shares `.layout-grid` with a second rail — the entry-detail `.toc` (Compass chapters/Journal entries with 3+ headings) or the Reviews/Journal `.upcoming` "in the works" rail — gets two treatments, both scoped by `:has()` so single-sidebar pages (Home, `.detail` without a TOC) are untouched: at `≥1300px` the container widens from `1180px` to `1600px` (`.reviews-main`'s `1fr` column grows for free; `.detail-layout`'s article track is bumped explicitly from `760px` to `960px`); at all widths both rails sit at `opacity: 0.7` until hovered/focused-within (`:is(.sidebar, .toc, .upcoming)`, each independently), so the central column reads as the visual focus. |
