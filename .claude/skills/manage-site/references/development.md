...
| **Login, accounts & the campfire** | Session seam: `src/lib/session.tsx` (`SessionProvider`/`useSession`, outermost in `App.tsx`; owns `localStorage['gc-session']` and the dialog state). UI: `src/components/UserButton.tsx` (header, between the language and palette switchers), `src/components/LoginDialog.tsx` (modal), `src/pages/Account.tsx` (`/account`), `src/components/AccountFields.tsx` (right-rail panel). Types: `LoginResponse`/`MagicRequestPayload`/`MeResponse`/`Member`/`UpdateMePayload` in `lib/api.ts`. Styles: `.user-toggle`/`.login-*`/`.campfire*`/`.camper*`/`.page-head-row`/`.rail-toggle`/`.account-fields`/`.field*`/`.avatar-img`. Strings: `login.*`/`account.*` (en+ru). | All of it renders **only when the API answers** (`backendUp`: `/healthz` probe signed out, `/auth/me` signed in) — the pure static site shows none of it. Signed out the button opens the dialog — **passwordless-first**: email in, `POST /auth/magic` out, dialog flips to a "check your inbox" state; the emailed link lands as `<site>/#magic=<token>`, which `session.tsx` consumes (on mount AND on hashchange — pasting the link into an already-open tab works too), scrubs from the URL, and trades at `/auth/magic/verify` for a session (first sign-in creates a **viewer** account named after the email's local part — there is no separate registration form; a dead/expired token just reopens the dialog). A password form sits behind a toggle for accounts that have one (bootstrap admin). Signed in it shows the user's initial (or avatar image) and links to `/account`: the campfire, with every registered user placed around the fire by trig (`Account.tsx` computes seat angles; avatar = the user's `avatarUrl` image if set — cropped/zoomed via `.avatar-img` (`object-fit: cover; transform: scale(2)`), same crop approach as `.epic-row-img` — else the display-name initial cycling the pastel slots; `you` highlighted with the accent). `AccountFields` (name, avatar URL, email, password; Save appears only once a field differs from the last-saved values; role is shown but disabled — admin-only to change) is **one JSX tree, always mounted** as the standard `.reviews-layout` right rail (same as Reviews/Journal/Compass) — always visible on desktop. Below 900px the breakpoint is pure CSS, no JS branch: `.rail-toggle` (hidden by default, shown ≤900px) is a `.user-toggle` pencil button next to the title in a `.page-head-row`; clicking it flips `fieldsOpen` state in `Account.tsx`, which adds `.is-open` to `<AccountFields>` — `.account-fields` itself is `display:none` below 900px unless `.is-open` is present. `.page-head-row`/`.rail-toggle` are the same shared classes Reviews/Journal now use for their own Upcoming-rail toggle (see the Upcoming row) — one mobile-collapsible-rail pattern, several pages. The form calls `PUT /api/users/me` (session-authed, any role) and syncs the result back into `session.me` via `useSession().updateMe()`; email conflicts surface a 409 inline. Sign-out button sits below the scene. Login/save errors localize by HTTP status (401/409/400/429/503 — 503 = no email transport configured on the BE, "links unavailable"; live transport is Postmark's HTTP API, SMTP is the dormant fallback — see `backend/internal/mail`). In `task dev` the BE runs `SMTP_HOST=log`: the magic email (link included) prints to the api container's stdout — copy the link into the browser to sign in, no credentials. WebAuthn passkeys are future work, gated on the permanent domain (`context/auth/auth-paths.md`). |
...
- **Login is a header feature; edit mode rides on it.** The account button
  ...
  answers; it opens the `LoginDialog` (magic-link email first, password
  fallback) and, signed in, links to `/account` (the campfire; sign-out
  lives there). The session seam is `src/lib/session.tsx` — token in
  ...
  magic-link sign-in creates a viewer); in `task dev` the bootstrap admin is
  `dev@local` / `dev-password` (nuke `backend/data/` to re-bootstrap) and
  magic emails print to the api container's stdout (`SMTP_HOST=log`).
