...
| Reviews / Compass / Journal / pages content | `content/locales/<lng>/…` | Markdown+YAML; see `content-editing.md`. **Compass** chapters live in per-epic subfolders (`compass/founder-guide/`, `compass/homeopathy/`, `compass/herbalism/`, `compass/trophology/`, `compass/inside-websites/`) — organizational only, `content.ts` globs `compass/**/*.md`, slug = filename. **Journal** entries are flat (`journal/*.md`, globbed `journal/*.md`). (`guides/` was renamed to `compass/` 2026-07-06 — no `/guides` route or dir remains.) |
...
| **Journal list + year filter** | `src/pages/Journal.tsx` + `src/components/JournalRow.tsx` + `.journal-layout`/`.journal-main`/`.journal-list`/`.journal-row*`/`.journal-years`/`.journal-year`/`.journal-template-btn` in `styles.css` | The `/journal` listing: a `.journal-layout` grid (main column + a `clamp(180px,20vw,240px)` right rail), mirroring `.reviews-layout`. **Right rail = a TOC-like year filter** (`.journal-years`): an "All" button + one button per distinct entry year (desc); `useState` holds the active year (`undefined` = all), filtering the list. `JournalRow` is a plain **text row (no thumbnail)** — muted date, title, 2-line-clamped excerpt; the whole row is the `<Link to={/journal/<slug>}>`. Entries sort date-desc (`content.ts`). **≤900px** the layout flexes to a column and the year rail rides above the list (`order:-1`, horizontal wrap-row). Above the list sits the **copy-template button** (next row). |
| **Journal "copy a blank template" button** | labelled `CopyButton` in `src/pages/Journal.tsx`; template source `content/shared/journal-template.<locale>.md` → `getJournalTemplate(locale)` in `content.ts`; `.journal-template-btn` + `.copy-btn-label` in `styles.css` | Copies a generic (topic-agnostic) note scaffold to the clipboard for the owner to start a new entry. **Single source:** one raw markdown template per locale under `content/shared/` (en fallback), globbed like everything else. `CopyButton` gained an optional `label` prop → labelled action button (icon + text, swaps to "Copied"); icon-only stays the default (wallets). Future "even more generic for readers" template = just another variant of this file/button. |
| **Compass section / epics (courses)** | `src/pages/Compass.tsx` + `.epic-strip`/`.epic-thumb*`/`.epic-blurb`/`.compass-provenance` in `styles.css`; config in `site.yaml` `epics:` | The Compass section is user-facing **"Compass" / "Путь"** (nav label + `compass.title`); **route + content dir are `/compass`** (renamed from `/guides` 2026-07-06). Under the page header sits the **`compass.provenance` banner** — the "openly computer-assisted" disclosure (Compass is the one AI-drafted section). Epic thumbnails act as tabs; the row list below shows chapters whose **first tag** equals the selected epic's `tag`, with the epic's `blurb` as a course intro. First epic = default. Membership is data (first tag); display metadata (title, image, blurb) is `site.yaml` `epics:` (`GuideEpic` in `lib/types.ts`). The word "Epic" never appears in UI. Within the list, chapters with an optional `chapter: N` sort ascending (course order) ahead of un-chaptered ones, which keep date-desc — see "Compass chapter ordering" below. |
| **Compass chapter ordering** | `chapter?: number` on `CompassEntry` (`lib/types.ts`); sort in `Compass.tsx`; label in `CompassRow.tsx` (epic listing), `CompassCard.tsx` (homepage teaser) + `EntryDetail.tsx` via `t('compass.chapter', { n })` | Optional frontmatter field for course-like epics — every course epic uses it (founder-guide got `chapter:` 1–5 in the 2026-07-05 course retrofit; trophology is `chapter:` 1–5). `Compass.tsx` sorts the epic-filtered list: chaptered entries ascending by `chapter`, before/apart from un-chaptered ones (which keep the existing date-desc order — a stable sort). All three surfaces show a "Chapter N" / «Глава N» tag when `chapter` is present. Journal entries share `EntryDetail` but never show a chapter tag (`kind="journal"`). |
