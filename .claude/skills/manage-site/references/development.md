...
| **Live-edit: edit mode + git-backed editor** | Gate: `src/lib/editMode.tsx` (`EditModeProvider`/`useEditMode`, a thin consumer of `useSession`). Editor: `src/lib/contentEditor.tsx` (`ContentEditorProvider`/`useContentEditor` — `openField`/`openDraft`), mounted above `<Routes>`. Buttons: `src/components/EditButton.tsx` (`icon="pencil"` in-place edit, `icon="plus"` draft). Addresses: `EditRef` in `lib/types.ts`, minted only by provenance getters in `lib/content.ts` — `getProductFile`/`getJournalFile` for products/journal (locale-first, falling back to en, mirroring `entryFor`'s fallback; Compass isn't wired — its epic subfolders don't fit the flat `products`/`journal` convention). BE side: `backend/internal/content` (storage seam) behind `backend/internal/httpapi` (session-authed routes). Styles: `.edit-btn`/`.wip-edit`/`.content-editor*` in `styles.css`. `editor.*` UI strings (en+ru). | **Readers never see any of this** — buttons render only in edit mode (a signed-in admin/editor session with a configured storage backend; see the login gotcha below). `openField({title, ref, onSaved})`: fetches the CURRENT file via `/api/content/file`, shows just the scalar, Save does **CST-level byte-preserving YAML surgery** (`Parser`/`Composer`/`CST.setScalarValue` from the existing `yaml` dep — the Document API re-folds long block scalars; don't switch back). For a plain YAML file (site.yaml/themes.yaml — no `EditRef` actually wired to one yet) the surgery runs on the whole file; for a markdown content file it runs on just the `---` frontmatter block (`FRONTMATTER_RE` in `contentEditor.tsx`, same delimiter shape as `content.ts`'s own `parseFrontmatter` but capturing the delimiters too, since the edit needs to splice the untouched body back byte-for-byte) — `docs.length !== 1` guards against ever mis-editing a multi-document blob. Either way it posts the whole file + sha to `/api/content/save`; 409 ⇒ re-fetch, re-apply, one retry; success shows "published, live in ~2 min" and calls `onSaved` (optimistic UI — the static `content.ts` data itself only catches up once that deploy lands). **Known limit:** this can only edit a scalar that already exists in the frontmatter — there's no "insert a missing key" path, so it can't set `state` on a post that has no `state:` line at all (fine for the one field wired today, since upcoming posts always have the line). `openDraft({title, path, initialValue, message})`: composes a new content file — a pre-flight GET refuses overwriting (404 = good case). Every save is a git commit (`content: … via portal`) → the normal Pages deploy publishes it (~2 min); git stays the only source of truth. **Wired today:** on `ReviewDetail.tsx`/`EntryDetail.tsx` (journal only — Compass ignores `state`), the "In the works" tag gets a `.wip-edit` pencil `EditButton` next to it (shown only when `state === 'upcoming'` — the key is guaranteed to exist) that opens a field editor for `state`; `onSaved` sets local component state so the tag disappears immediately rather than waiting for the actual deploy. On the `Upcoming` rail — rows link to the post's real URL (WIP preview), and one ＋ button under the list queues a NEW upcoming post: `window.prompt` for the title, then `openDraft` saves `content/locales/<locale>/{products,journal}/<slug>.md` (Cyrillic-aware `slugify`) from `draft.template` (the kind's blank scaffold, passed in by `Reviews.tsx`/`Journal.tsx` via `getReviewTemplate`/`getJournalTemplate`), which already carries `state: upcoming`. The old anonymous "Contribute!" copy-template button is gone — queuing a draft is edit-mode-only now, since the ＋ button covers it directly. |
