...
- **Login is a header feature; edit mode rides on it.** The account button
  ...
  answers; it opens the `LoginDialog` (sign-in / create-account) and, signed
  in, links to `/account` (the campfire; sign-out lives there). The session
  seam is `src/lib/session.tsx` — token in `localStorage['gc-session']`,
  validated against `/api/auth/me` on every load: a 401 clears it, BE-down
  hides all account chrome. **Edit mode** is simply `me.editing === true`
  ...
  Accounts live in the backend DB (roles `admin`/`editor`/`viewer`;
  self-registration creates viewers); in `task dev` the bootstrap admin is
